...
const SnakeGame = ({ onClose, onScore }: { onClose: () => void; onScore: (score: number) => void }) => {
  ...
  // ---- backend save
  async function saveScoreToBackend(finalScore: number) {
    try {
    ...
    <div className="fixed inset-0 bg-background/95 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <Card className="p-6 bg-card/90 backdrop-blur-sm border-2 border-neon-cyan crt-monitor">
        <div className="text-center mb-4">
          <h2 className="font-arcade text-2xl text-neon-cyan text-glow mb-1">SNAKE</h2>
          <div className="font-cyber text-neon-yellow">PLAYER: {playerName || '—'}</div>
          <div className="font-cyber text-neon-yellow mt-1">SCORE: {score}</div>
        </div>
        ...
        <div className="relative mb-4">
          ...
          {/* Pre-game overlay: ask name */}
          {!gameRunning && !gameOver && (
            <div className="absolute inset-0 flex items-center justify-center bg-background/80">
              <div className="text-center space-y-3">
                <div className="font-arcade text-neon-cyan text-glow">ENTER PLAYER NAME</div>
                <input
                  className="font-cyber px-3 py-2 rounded-md bg-black/60 border border-neon-cyan/60 outline-none focus:ring-2 focus:ring-neon-cyan text-neon-yellow placeholder:text-foreground/50"
                ...
                />
                <div>
                  <Button variant="arcade" onClick={startGame} disabled={!canStart} className="font-cyber disabled:opacity-50">
                    START GAME
                  </Button>
                </div>
                <div className="text-xs text-foreground/60 font-cyber">Tip: Use arrow keys to control the snake</div>
              </div>
            </div>
          )}
        ...
        <div className="flex justify-center gap-2">
          <Button variant="ghost" onClick={onClose} className="font-cyber">BACK TO ARCADE</Button>
          {gameRunning && (
            <Button
              variant="destructive"
              onClick={() => { setGameRunning(false); setGameOver(true); saveScoreToBackend(score); }}
